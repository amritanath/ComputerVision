...
for i in range(len(t)) :
    if t[i]>0.008856 :
        L.append(   int(round((116*(pow(t[i] ,(1/3)))) - 16)))
    else :
        L.append( int(round(903.3 * t[i])))
...
for i in range(len(LSort)):
    if(LSort[i] < min):
        ...
        LSort[i] = round(LSort[i])

for i in range(len(L)):
    Ltemp.append(LSort[i])
...
for i in counts:
    if abc!=0:
        farr[i] = int(((prev+fi[i])/2)*(101/fi[lastvalue]))
        prev = farr[i]
    else:
        farr[i] = int(((0+fi[i])/2)*(101/fi[lastvalue]))
        prev = farr[i]
        abc+=1
#print('------------------------------------------------')
#print(farr)
